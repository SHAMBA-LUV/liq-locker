...
import { ETHEREUM, Reader, Signer, formatUnits, untilWords, modules, } from "./app.js";
 ...
 */
export function mountPanel(root, props) {
    ...
    async function refresh() {
        try {
            count = (await reader.call("lock_count"))[0];
        }
        catch { /* context only */ }
        try {
            ...
            status = { kind: "err", text: e.message };
        }
        render();
    }
    ...
    function render() {
        ...
        grid.append(cell("principal", formatUnits(view.amount)), cell("collectable", formatUnits(view.collectable), view.collectable > 0n), cell("opens in", untilWords(view.unlockAt)), cell("state", view.withdrawn ? "withdrawn" : view.isLocked ? "locked" : "matured"));
        card.append(grid);
        // ── COLLECT
        const why = view.withdrawn ? "this lock has been withdrawn"
            : view.collectable === 0n ? "nothing has accrued yet"
                : !signer ? "connect a wallet to sign"
                    : signer.account.toLowerCase() !== view.beneficiary.toLowerCase() ? "only the beneficiary may collect"
                        : null;
        const btn = el("button", "lk-collect");
        btn.append(el("span", "lk-collect-main", VERBS.collect.label));
        btn.append(el("span", "lk-collect-sub", why ?? `${formatUnits(view.collectable)} — ${VERBS.collect.hint}`));
        ...
        card.append(el("p", "lk-fine", "Collect takes the reflection growth only. The principal stays locked until maturity."));
        // ── wallet, offered rather than demanded
        if (!signer) {
            const connect = el("button", "lk-connect", "Connect wallet to sign");
        ...
        if (status.text) {
            ...
            if (status.hash) {
                const a = el("a", "lk-link", "view transaction");
        ...
        for (const m of modules()) {
            ...
            m.render({ reader, signer, lock: view, cfg })
                ...
                for (const row of out.rows ?? []) {
                    ...
                    if (row.href) {
                        const a = el("a", "lk-link", row.value);
                        a.href = row.href;
                        a.target = "_blank";
                        a.rel = "noopener";
                        r.append(a);
                        ...
                        r.append(el("b", "lk-value", row.value));
                    slot.append(r);
                }
                if (out.note)
                    slot.append(el("p", "lk-fine", out.note));
            })
